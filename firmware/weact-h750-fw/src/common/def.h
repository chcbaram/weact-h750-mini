...
typedef struct 
{
  ...
  //-- firm_size 는 링커가 링크 시점에 아는 값이다 (_fw_flash_end - _fw_flash_begin).
  //
  //   CRC 는 자기 이미지 안에 넣을 수 없지만(닭-달걀) 크기는 넣을 수 있다.
  //   이 비대칭 덕분에, 태그가 없는 이미지(아두이노 코어 빌드 등)도 크기를
  //   스스로 신고할 수 있고 부트로더가 CRC 를 계산해 태그를 만들어 줄 수 있다.
  //   0 이면 "크기 미신고" 로 본다.
  uint32_t firm_size;
} firm_ver_t;
