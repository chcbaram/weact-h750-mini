...
void HAL_SPI_MspInit(SPI_HandleTypeDef* spiHandle)
{
  ...
  */
    ...
    /**SPI4 GPIO Configuration  (WeAct STM32H7XX Board V1.2, 06-TFT-LCD.SchDoc)
    PE12     ------> SPI4_SCK   (LCD_SCL)
    PE14     ------> SPI4_MOSI  (LCD_SDA)

    LCD 는 4-wire SPI 라 MISO 가 없다. CS(PE11) 와 DC(PE13) 는 일반 GPIO 로 민다.
    */
    ...
    /* DMA1_Stream1 = SPI4_TX. Stream0 은 USART1 RX 가 쓰고 있다. */
...
{
  //-- 원본은 SPI3 와 비교하는 오타가 있어 사실상 아무것도 하지 않았다.
  //   핀 목록도 MspInit 과 반드시 일치해야 한다 (QSPI 쪽에서 같은 실수로 물렸다).
  if (spiHandle->Instance == SPI4)
  {
