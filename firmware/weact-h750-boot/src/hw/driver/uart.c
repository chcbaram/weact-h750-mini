...
// 이 보드는 D-Cache 를 켜고 쓰므로 DMA 가 건드리는 rx_buf 는 반드시
// non-cacheable 영역(.non_cache = D2 SRAM)에 있어야 한다. 링커스크립트에
// .non_cache 섹션을 선언해 두었다. 선언하지 않으면 orphan 이 되어 startup 의
// .data 복사에도 .bss 클리어에도 포함되지 않고 부팅 시 쓰레기값이 남는다.
...
static uart_hw_t uart_hw_tbl[UART_MAX_CH] = 
  {
    {"USART1 SWD   ", USART1, &huart1, false, NULL},   // PA9 / PA10
  ...
  switch(ch)
  {
    case _DEF_UART1:
      ...
      if (ret_hal == HAL_OK)
      {
        ...
        uart_tbl[ch].qbuffer.in  = uart_tbl[ch].qbuffer.len - __HAL_DMA_GET_COUNTER(uart_tbl[ch].p_huart->hdmarx);
...
void HAL_UART_MspInit(UART_HandleTypeDef* uartHandle)
{
  ...
  if (uartHandle->Instance == USART1)
  {
    ...
    /* PA9 -> USART1_TX, PA10 -> USART1_RX  (P1/P2 헤더로 나와 있는 핀) */
    ...
    /*
     * RX 는 순환 DMA 로 링버퍼에 계속 받아둔다. 인터럽트는 쓰지 않고
     * uartAvailable() 에서 NDTR(남은 전송 개수)을 읽어 쓰기 위치를 역산한다.
     *
     * 수신 버퍼는 .non_cache(D2 SRAM) 에 있다. D-Cache 를 켠 상태에서 DMA 가
     * 캐시 가능한 메모리에 쓰면 CPU 가 옛 캐시 라인을 읽는다. DMA1 은 D2
     * 도메인이라 0x30000000 에 직접 접근할 수 있다.
     */
