...
#### 부트로더도 자기가 켠 것은 끄고 넘긴다 (V260830R8)

위 규약("페리페럴은 앱이 초기화한다")은 유지하되, **부트로더가 시작한 것은
부트로더가 멈춘다.** NVIC 만 막고 페리페럴을 살려 보내는 것이 어중간했다.

`bspDeInit()` 이 이제 이 순서로 한다.

```
1. NVIC 마스크 (ICER)
2. **SPI4 + DMA1 을 RCC 로 리셋**   <- 새로 추가
3. NVIC pending clear (ICPR)        <- 리셋 과정에서 뜬 것까지 지우려면 뒤여야 한다
4. SysTick 정지
5. 캐시 clean/invalidate
```
...
리셋을 40회 반복하며 앱의 생존을 확인했다. 판정은 **PC 를 300ms 간격으로 두 번
읽어 바뀌는가** 로 했다(같은 값이면 굳은 것). 신뢰할 수 있는 지표는 **폴트가
잡힌 것**뿐이다.

```
CFSR  0x00000400   BFSR bit10 IMPRECISERR (BFARVALID=0)
...
**60% 줄었지만 없어지지 않았다.** 남은 10% 는 앱 쪽 초기화 순서로 보인다 —
`HAL_SPI_Init()` 은 H7 에서 **IER 도 IFCR 도 건드리지 않으므로**, 앱이 자기
쪽에서도 지워야 한다(아두이노 세션이 커밋 `93a99a7` 로 고쳤다. 미검증).

> **판정 기준 주의.** "PC 가 QSPI 안에 있으면 성공" 은 **틀린 기준이다.**
> `Default_Handler` 도 QSPI 안에 있다. 이 기준으로 30/30, 60/60 을 재고
> "문제 없음" 이라고 잘못 결론냈었다.
>
> "PC 가 두 번 다 같으면 굳음" 도 **위양성 10%** 가 있다. 정상 앱이 2명령
> 대기 루프에 있으면 같은 값이 나온다(실측 1/10). 폴트 레지스터를 함께 봐야 한다.
