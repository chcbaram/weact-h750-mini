...
## 배선은 되어 있으나 솔더 브리지로 끊긴 기능

이 보드는 **기능을 회로에 넣어두고 솔더 브리지를 열어서 끊어놓은** 자리가 있다.
헤더에 핀이 나와 있고 데이터시트에도 있으니 "된다" 고 착각하기 쉽다.
**둘 다 브리지를 닫아야 동작한다.**

| 브리지 | 연결 | 출하 상태 | 닫지 않으면 |
|---|---|---|---|
| **SB1** | 카메라 헤더 PWDN ↔ **PA7** | **열림** | **카메라 센서를 확실히 리셋할 방법이 없다** |
| **SB2** | µSD 카드 검출 ↔ **PD4** | **열림** | 카드 삽입을 감지할 수 없다 (핀이 뜬다) |

### SB2 — µSD 카드 검출

PD4 가 뜬 상태다. 실측으로 확인했다(아두이노 세션) — 카드를 꽂은 채로 내부
풀업이면 HIGH, 풀다운이면 LOW 로 따라온다. 즉 **핀이 아무데도 안 닿아 있다.**

그래서 아두이노 코어의 SD 드라이버는 `HW_SD_DETECT_NONE` 으로 두고
`sdIsDetected()` 가 항상 true 를 돌려준다.

### SB1 — 카메라 PWDN

**이쪽이 더 고약하다.** 카메라 헤더에는 **리셋 라인이 아예 없고**, PWDN 만
SB1 을 거쳐 PA7 에 닿는다. 즉 SB1 이 열려 있으면 센서를 리셋할 수단이
**SCCB 로 보내는 `COM7_SRST` 소프트 리셋 하나뿐**이고, 그것만으로는 센서가
확실히 초기 상태로 돌아가지 않는다.

증상이 지독하다. **업로드는 MCU 만 소프트 리셋한다.** 센서 전원은 안 끊기므로
이전 실행이 남긴 레지스터 상태를 그대로 물고 다음 실행으로 넘어간다.

```
센서 소프트 리셋 후 5ms      3/5    40% 실패
센서 소프트 리셋 후 300ms    9/10   10% 실패
```

**"전원을 껐다 켜면 항상 된다"** 가 결정적 단서였다 (사용자 제보). 업로드로
재현되는 간헐 결함인데 원인이 MCU 밖에 있다.

SB1 을 닫으면 PA7 로 PWDN 을 토글해 센서를 확실히 리셋할 수 있다.
