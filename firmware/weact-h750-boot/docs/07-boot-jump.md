...
### `HW_BOOT_TRY_MAX` 는 왜 꺼져 있나

점프 직전에 올리고 앱이 `resetConfirmBoot()` 으로 되돌리는 방식이다. **앱이 그 함수를
부르지 않으면 정상 앱도 몇 번 부팅한 뒤 막힌다.** 아두이노 코어는 부르지 않는다.

그래서 기본값이 `0`(비활성)이다. 코드는 `#if HW_BOOT_TRY_MAX > 0` 로 들어가 있으므로,
자체 앱(`weact-h750-fw`)에 확인 호출을 넣은 뒤 켜면 된다.

폴트 카운터로 잡히지 않는 것은 "폴트 없이 스스로 리셋을 반복하는 앱"뿐이다.
